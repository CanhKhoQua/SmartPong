...
batch_size = 10  # Number of episodes to play before updating the network weights
...
render = False  # Whether to display the game screen during training
...
def save_progress(file_path, model, grad_buffer, m_cache, v_cache, episode_number, running_rewards, running_reward, episode_rewards, epsilon):
    progress = {
        'model': model,
...
# Setup for plotting
def save_rewards_data(episode_rewards, running_rewards, save_path='episode_and_running_rewards_line_plot.png'):
    if episode_rewards and running_rewards:
        plt.figure(figsize=(12, 6))
        
        episodes = range(1, len(episode_rewards) + 1)  # Adjusted for 1-based indexing in plotting
        
        # Line plot for episode rewards
        plt.plot(episodes, episode_rewards, label='Reward per Episode', color='tab:blue', alpha=0.6, marker='o', markersize=4, linestyle='-')
        
        # Line plot for running mean of rewards to visualize the performance trend over time
        plt.plot(episodes, running_rewards, label='Running Mean', color='tab:red', linewidth=2, linestyle='--')
        
        # Enhancing the plot
        plt.title('Episode Rewards vs. Running Mean Rewards')
        plt.xlabel('Episode')
        plt.ylabel('Reward')
        plt.legend(loc='upper left')
        
        # Improving x-axis labels for clarity
        if len(episodes) > 50:  # Adjust labels if there are many episodes to avoid clutter
            plt.xticks(range(0, len(episodes) + 1, len(episodes) // 10))
...
if resume:
    ...
    if progress:
        ...
        running_reward = progress['running_reward']
...
plot_update_freq = 100  # Frequency of plot updates, e.g., every 100 episodes

training_episode = 15000 # stop after 1000 episodes
    ...
    if done:
        ...
        if episode_number % batch_size == 0:
            for k, v in model.items():
                ...
                grad_buffer[k] = np.zeros_like(v)  # Reset gradient buffer after updating parameters
        ...
        if episode_number % 100 == 0:
            save_progress(save_file_path, model, grad_buffer, m_cache, v_cache, episode_number, running_rewards, running_reward, episode_rewards, epsilon)
